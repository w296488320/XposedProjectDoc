...
public class StatusInstallerFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        //根据 本地是否含有 必须的zip判断显示
        refreshZipViews(v);

        // Disable switch
        // 激活 选择框
        final SwitchCompat disableSwitch = (SwitchCompat) v.findViewById(R.id.disableSwitch);
        //判断状态 /data/user_de/0/de.robv.android.xposed.installer/conf/disabled 是否存在进行判断
    ...
    private void refreshInstallStatus() {
        ...
        //获取 获取Xposed的一些信息 用于显示和判断 主要保存在 system/xposed.prop 没有安装返回-1
    ...
    private void download(Context context, String title, FrameworkZips.Type type, final RunnableWithParam<File> callback) {
        ...
        Log.e("下载地址  ", "download: "+zip.url+"  下载 title "+zip.title);
        new DownloadsUtil.Builder(context)
                .setTitle(zip.title)
                //"http://dl-xda.xposed.info/framework/sdk22/x86/xposed-v89-sdk22-x86.zip"
                .setUrl(zip.url)
